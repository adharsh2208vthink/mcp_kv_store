...
program
  .name('mcp-kv-server')
  .description('A standalone Key-Value store MCP server with Redis backend')
  .version('1.0.0')
  .option('--redis-url <url>', 'Redis connection URL', process.env.REDIS_URL || 'redis://localhost:6379')
...
async function startServer() {
  ...
  await kvStore.init();
  console.log(`✅ KV Store initialized with Redis at ${config.redisUrl}`);
  ...
  console.log(`✅ KV Store MCP Server with Redis backend started`);
